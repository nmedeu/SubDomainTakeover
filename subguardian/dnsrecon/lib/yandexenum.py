...
def scrape_yandex(dom):
    ...
    for _ in searches:
        ...
        if re.search('enter_captcha_value', data):
            error("Yandex has detected the search as 'bot activity', stopping search...")
            return unique(results)

        results.extend(re.findall(r'([a-zA-Z0-9\-\.]+' + re.escape(dom) + ')/?', data))

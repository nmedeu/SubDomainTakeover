...
class DnsHelper:
    ...
    def get_soa(self):
        ...
        try:
            ...
            error(f'Exception "{e}" while resolving SOA record.')
            error(f'Error while resolving SOA while using {self._res.nameservers[0]} as nameserver.')
            return []
    ...
    def zone_transfer(self):
        ...
        try:
            ...
            error('Could not obtain the domains SOA Record.')
            return
        ...
        try:
            ...
            error(f'Could not Resolve NS Records: {e}')
        ...
        for ns_srv in ns_records:
            ...
            if not self.check_tcp_dns(ns_srv):
                error(f'Zone Transfer Failed for {ns_srv}!')
                error('Port 53 TCP is being filtered')
            ...
            try:
                ...
                for name, rdataset in zone.iterate_rdatasets(dns.rdatatype.CERT):
                    for rdata in rdataset:
                        ...
                        print_status(f'\t CERT {rdata.to_text()}')
                        zone_records.append(
                            {
                                'zone_server': ns_srv,
                                'type': 'CERT',
